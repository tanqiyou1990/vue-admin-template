...
export default {
  ...
  methods: {
    ...
    saveCard() {
      if (this.defaultCardInfo.icoUrl == null || this.defaultCardInfo.text == null || 
      this.defaultCardInfo.icoUrl == '' || this.defaultCardInfo.text == '') {
        this.$message({
          message: '主题内容为必填项!',
          type: 'warning'
        })
    ...
    initByRelease(releaseNum) {
      this.listLoading = true
      loadHomeByRelease(releaseNum,3)
        .then(res => {
          this.listLoading = false
          this.form = res.data
